...
class MP_Debug_Model_Service
{
    ...
    {
        $this->getConfig()->saveConfig(MP_Debug_Helper_Data::DEBUG_OPTION_FORCE_VARIEN_PROFILE_PATH, (int)$isEnabled);
    }
    ...
    {
        if (!Mage::helper('mp_debug')->isMagentoEE()) {
    ...
    public function searchConfig($query)
    {
        ...
        $configArray = Mage::helper('mp_debug')->xml2array($this->getConfig()->getNode(), $configArray);
    ...
    {
        $table = Mage::getResourceModel('mp_debug/requestInfo')->getMainTable();
    ...
    public function getFileUpdatesWithHandle($handle, $storeId, $area)
    {
        ...
        $updateFiles = Mage::helper('mp_debug')->getLayoutUpdatesFiles($storeId, $area);
