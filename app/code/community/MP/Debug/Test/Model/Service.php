...
/**
 * Class MP_Debug_Test_Model_Service
 *
 * @covers MP_Debug_Model_Service
 * @codeCoverageIgnore
 */
class MP_Debug_Test_Model_Service extends EcomDev_PHPUnit_Test_Case
{

    public function testConstruct()
    {
        $model = Mage::getModel('mp_debug/service');
        $this->assertNotFalse($model);
        $this->assertInstanceOf('MP_Debug_Model_Service', $model);
    }
    ...
    public function testFlushCache()
    {
        ...
        $model = $this->getModelMock('mp_debug/service', array('getCacheInstance'));
    ...
    public function testGetModuleConfigFilePath()
    {
        ...
        $configMock->expects($this->once())->method('getModuleConfig')
            ->with('MP_Module')->willReturn(new Varien_Simplexml_Element('<config><a/></config>'));

        $model = $this->getModelMock('mp_debug/service', array('getConfig'));
        $model->expects($this->any())->method('getConfig')->willReturn($configMock);

        $actual = $model->getModuleConfigFilePath('MP_Module');
        $this->assertEquals('etc/modules/MP_Module.xml', $actual);
    }
    ...
    public function testGetModuleConfigFilePathNotFound()
    {
        ...
        $configMock->expects($this->any())->method('getOptions')->willReturn($configOptions);
        $configMock->expects($this->once())->method('getModuleConfig')
            ->with('MP_Module')->willReturn(null);

        $model = $this->getModelMock('mp_debug/service', array('getConfig'));
        $model->expects($this->any())->method('getConfig')->willReturn($configMock);

        $model->getModuleConfigFilePath('MP_Module');
    }


    public function testSetModuleStatus()
    {
        $configXml = (object)(array('modules' => (object)array('MP_Module' => (object)array('version' => true))));

        $model = $this->getModelMock('mp_debug/service', array('getModuleConfigFilePath', 'loadXmlFile', 'saveXml'));
        $model->expects($this->once())->method('getModuleConfigFilePath')->with('MP_Module')->willReturn('etc/MP_Module.xml');
        $model->expects($this->once())->method('loadXmlFile')->with('etc/MP_Module.xml')->willReturn($configXml);
        $model->expects($this->once())->method('saveXml')->with($configXml, 'etc/MP_Module.xml')->willReturn(true);

        $model->setModuleStatus('MP_Module', false);
        $this->assertEquals('false', (string)$configXml->modules->MP_Module->active);
    }
    ...
    public function testSetModuleStatusWithoutPermissions()
    {
        ...
        $model->expects($this->once())->method('getModuleConfigFilePath')->with('MP_Module')->willReturn('etc/MP_Module.xml');
        $model->expects($this->once())->method('loadXmlFile')->with('etc/MP_Module.xml')->willReturn($configXml);
        $model->expects($this->once())->method('saveXml')->with($configXml, 'etc/MP_Module.xml')->willReturn(false);
    ...
    {
        $actual = Mage::getModel('mp_debug/service')->getLocalXmlFilePath();
    ...
    public function testSetSqlProfilerStatusDisable()
    {
        ...
        $model = $this->getModelMock('mp_debug/service', array('getLocalXmlFilePath', 'loadXmlFile', 'saveXml'));
    ...
    public function testVarienProfilerStatus()
    {
        ...
        $configMock->expects($this->once())->method('saveConfig')
            ->with('mp_debug/options/force_varien_profile', 1);
    ...
    public function testSetTemplateHints()
    {
        ...
        $model = $this->getModelMock('mp_debug/service', array('getConfig', 'deleteTemplateHintsDbConfigs'));
        $model->expects($this->any())->method('getConfig')->willReturn($configMock);
    ...
    public function testSearchConfig()
    {
        ...
        $helper = $this->getHelperMock('mp_debug', array('xml2array'));
        $this->replaceByMock('helper', 'mp_debug', $helper);
        $helper->expects($this->once())->method('xml2array')
            ->with($configNodeElement)
            ->willReturn(array(
                'modules/MP_Module/version'   => '1.0.0',
                'events/cool'                    => '1',
    ...
    public function testDeleteTemplateHintsDbConfigs()
    {
        ...
        Mage::getModel('mp_debug/service')->deleteTemplateHintsDbConfigs();
    }


    public function testPurgeAllProfiles()
    {
        $resourceMock = $this->getResourceModelMock('mp_debug/requestInfo', array('getMainTable'));
        $this->replaceByMock('resource_model', 'mp_debug/requestInfo', $resourceMock);
        ...
        $actual = Mage::getModel('mp_debug/service')->purgeAllProfiles();
        $this->assertEquals(2, $actual);
    }


    /**
     * @covers MP_Debug_Model_Service::getFileUpdatesWithHandle
     */
...
XML;
        ...
        $helperMock = $this->getHelperMock('mp_debug', array('getLayoutUpdatesFiles'));
        ...
        $model = $this->getModelMock('mp_debug/service', array('loadXmlFile'));
    ...
    /**
     * @covers MP_Debug_Model_Service::getDatabaseUpdatesWithHandle
     */
    public function testGetDatabaseUpdatesWithHandle()
    {
        ...
        $actual = Mage::getModel('mp_debug/service')->getDatabaseUpdatesWithHandle('some_handle', 10, 'adminhtml');
        $this->assertCount(2, $actual);
